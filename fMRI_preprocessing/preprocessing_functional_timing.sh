...
#   1) When the scanner is started or stopped, the mriplayer script continues to check if the TTL pulses are really coming for 100 ms
...
# v2.1 - Rewound to account for the missed time when the scanner was dropped but mistakenly fast-forwarded sometimes
# v2.2 - Rewound
...
do
  ...
  do
    ...
    mv files/timeseries_files/"$subject"_task-"$movie"_run-0?_bold_tshift_despike_reg_al_mni_mask_"$filename".nii.gz ./
    # NOTE
    # Add TRs where dropped
    # In the v1 of the movie script, we always lost one TR when the scanner is stopped
    # We also lost whatever bit of the movie the participant watched in that TR
    # So this adds the tr back on
    # It does so by:
    ...
    # It also makes the transition between TRs less abrupt, more smooth so that preprocessing and various analysis are less affected
    ...
    # Expect errors from this part of the loop as it will try to make a file from a run that doesn't exit - this is fine but needs fixing...
    ...
    do
      ...
      rm ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_last_tp.nii.gz
      rm files/timeseries_files/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_last_tp.nii.gz
      3dcalc \
        -a ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename".nii.gz[$] \
        -datum  float -expr "a*step(abs(a))" \
        -prefix ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_last_tp.nii.gz
      # Get first time point of next run
      next_run=`echo $((run + 1))`
      rm ./func/"$subject"_task-"$movie"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_first_tp.nii.gz
      rm files/timeseries_files/"$subject"_task-"$movie"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_first_tp.nii.gz
      3dcalc \
        -a      ./func/"$subject"_task-"$movie"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename".nii.gz[0] \
        -datum  float -expr "a*step(abs(a))" \
        -prefix ./func/"$subject"_task-"$movie"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_first_tp.nii.gz
      # Average these, i.e., the last tp and first tp
      rm ./func/"$subject"_task-"$movie"_run-0"$run"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz
      rm files/timeseries_files/"$subject"_task-"$movie"_run-0"$run"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz
      3dMean \
        -prefix ./func/"$subject"_task-"$movie"_run-0"$run"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz \
                ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_last_tp.nii.gz \
                ./func/"$subject"_task-"$movie"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_first_tp.nii.gz
      # Add the new slug to the end of the run timeseries
      rm ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz
      rm files/timeseries_files/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz
      3dTcat \
        -prefix ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz \
                ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename".nii.gz \
                ./func/"$subject"_task-"$movie"_run-0"$run"_run-0"$next_run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz
    ...
    # Explanation:
    # The amount of time dropped in run one will determine how far back to interpolate run 2 and so on
    ...
    # If the script comes up with >1000 this means we need to actually shift forward in time on the next TR whatever amount over 1000
    ...
    # This is b/c of the way the script we use monitors for a pulse every 50 ms and cycles through to make sure there was no prior pulse
    ...
    do
      ...
      then
        ...
        functional_run_lost_start_actual=functional_"$run"_lost_start_"$subject"
        run_interpolation_length=`echo ".${!functional_run_lost_start_actual}"`
        # See whether run 2 was rewound
        functional_2_rewind_value_actual=functional_2_rewind_"$subject"
        ...
        rm ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename""$file_ending"_interpolation_amount.nii.gz
        rm files/timeseries_files/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename""$file_ending"_interpolation_amount.nii.gz
        3dcalc \
          -a ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename""$file_ending".nii.gz[0] \
          -datum  float -expr "-1*(step(abs(a))*"$run_interpolation_length")" \
          -prefix ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename""$file_ending"_interpolation_amount.nii.gz
        # Do time shifting using the above file
        rm ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_timing.nii.gz
        rm files/timeseries_files/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_timing.nii.gz
        3dTshift \
          -voxshift ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename""$file_ending"_interpolation_amount.nii.gz \
          -prefix   ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename"_timing.nii.gz \
                    ./func/"$subject"_task-"$movie"_run-0"$run"_bold_tshift_despike_reg_al_mni_mask_"$filename""$file_ending".nii.gz
      fi
      if [ $run -eq  2 ]
      then
        # NOTE
        ...
        functional_2_rewind_value_actual=functional_2_rewind_"$subject"
        functional_2_rewind_value=`echo "${!functional_2_rewind_value_actual}"`
        functional_3_rewind_value_actual=functional_3_rewind_"$subject"
        ...
        # Make variable
        current_run_functional_run_lost_start_actual=functional_"$run"_lost_start_"$subject"
        prior_run_functional_run_lost_start_actual=functional_"$prior_run"_lost_start_"$subject"
        prior_run_functional_run_lost_end_actual=functional_"$prior_run"_lost_end_"$subject"
        # Get value
      ...
      then
        ...
        functional_3_rewind_value_actual=functional_3_rewind_"$subject"
        functional_3_rewind_value=`echo "${!functional_3_rewind_value_actual}"`
        functional_4_rewind_value_actual=functional_4_rewind_"$subject"
        ...
        current_run_functional_run_lost_start_actual=functional_"$run"_lost_start_"$subject"
        prior_run_functional_run_lost_start_actual=functional_"$prior_run"_lost_start_"$subject"
        prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_run"_lost_start_"$subject"
        prior_run_functional_run_lost_end_actual=functional_"$prior_run"_lost_end_"$subject"
        prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_run"_lost_end_"$subject"
        # Get value
        current_run_functional_run_lost_start=`echo "${!current_run_functional_run_lost_start_actual}"`
        prior_run_functional_run_lost_start=`echo "${!prior_run_functional_run_lost_start_actual}"`
      ...
      then
        ...
        functional_4_rewind_value_actual=functional_4_rewind_"$subject"
        functional_4_rewind_value=`echo "${!functional_4_rewind_value_actual}"`
        functional_5_rewind_value_actual=functional_5_rewind_"$subject"
        ...
        prior_run_functional_run_lost_start_actual=functional_"$prior_run"_lost_start_"$subject"
        prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_run"_lost_start_"$subject"
        prior_prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_prior_run"_lost_start_"$subject"
        # Make variable end
        prior_run_functional_run_lost_end_actual=functional_"$prior_run"_lost_end_"$subject"
        prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_run"_lost_end_"$subject"
        prior_prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_prior_run"_lost_end_"$subject"
        # Get value for start
        current_run_functional_run_lost_start=`echo "${!current_run_functional_run_lost_start_actual}"`
        prior_run_functional_run_lost_start=`echo "${!prior_run_functional_run_lost_start_actual}"`
        prior_prior_run_functional_run_lost_start=`echo "${!prior_prior_run_functional_run_lost_start_actual}"`
      ...
      then
        ...
        functional_5_rewind_value_actual=functional_5_rewind_"$subject"
        functional_5_rewind_value=`echo "${!functional_5_rewind_value_actual}"`
        functional_6_rewind_value_actual=functional_6_rewind_"$subject"
        ...
        prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_run"_lost_start_"$subject"
        prior_prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_prior_run"_lost_start_"$subject"
        prior_prior_prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_prior_prior_run"_lost_start_"$subject"
        # Make variable end
        prior_run_functional_run_lost_end_actual=functional_"$prior_run"_lost_end_"$subject"
        prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_run"_lost_end_"$subject"
        prior_prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_prior_run"_lost_end_"$subject"
        prior_prior_prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_prior_prior_run"_lost_end_"$subject"
        # Get value for start
        current_run_functional_run_lost_start=`echo "${!current_run_functional_run_lost_start_actual}"`
        prior_run_functional_run_lost_start=`echo "${!prior_run_functional_run_lost_start_actual}"`
        prior_prior_run_functional_run_lost_start=`echo "${!prior_prior_run_functional_run_lost_start_actual}"`
      ...
      then
        ...
        functional_6_rewind_value_actual=functional_6_rewind_"$subject"
        functional_6_rewind_value=`echo "${!functional_6_rewind_value_actual}"`
        functional_7_rewind_value_actual=functional_7_rewind_"$subject"
        ...
        prior_prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_prior_run"_lost_start_"$subject"
        prior_prior_prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_prior_prior_run"_lost_start_"$subject"
        prior_prior_prior_prior_prior_run_functional_run_lost_start_actual=functional_"$prior_prior_prior_prior_prior_run"_lost_start_"$subject"
        # Make variable end
        prior_run_functional_run_lost_end_actual=functional_"$prior_run"_lost_end_"$subject"
        prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_run"_lost_end_"$subject"
        prior_prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_prior_run"_lost_end_"$subject"
        prior_prior_prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_prior_prior_run"_lost_end_"$subject"
        prior_prior_prior_prior_prior_run_functional_run_lost_end_actual=functional_"$prior_prior_prior_prior_prior_run"_lost_end_"$subject"
        # Get value for start
        current_run_functional_run_lost_start=`echo "${!current_run_functional_run_lost_start_actual}"`
        prior_run_functional_run_lost_start=`echo "${!prior_run_functional_run_lost_start_actual}"`
        prior_prior_run_functional_run_lost_start=`echo "${!prior_prior_run_functional_run_lost_start_actual}"`
