...
# This script currently only words for up to four runs
  ...
  do
    ...
    do
      ...
      then
        ...
        # rm files/timeseries_files/media_"$run"_tshift_despike_reg_al_mni_mask_"$filename"_interpolation_amount.nii.gz
        # rm media_"$run"_tshift_despike_reg_al_mni_mask_"$filename"_slug_interpolation_amount.nii.gz
        # rm files/timeseries_files/media_"$run"_tshift_despike_reg_al_mni_mask_"$filename"_slug_interpolation_amount.nii.gz
        # 3dcalc \
        ...
        # 3dcalc \
        #   -a      media_"$run"_tshift_despike_reg_al_mni_mask_"$filename"_slug.nii.gz[0] \
        #   -datum  float -expr "-1*(step(abs(a))*"$run_interpolation_length")" \
        #   -prefix media_"$run"_tshift_despike_reg_al_mni_mask_"$filename"_slug_interpolation_amount.nii.gz
